...
from runtime_CLI import RuntimeAPI, PreType, thrift_connect, load_json_config
...
class SimpleSwitch:
    ...
    def table_add(self, table, action, values, params, priority):
        ...
        self.api.do_table_add(
            '{} {} {} => {}{}'.format(table, action, ' '.join(values), ' '.join([str(x) for x in params]), priority_str))
    ...
    # This functions replaces the original send_and_check_only_1_packet() of p4pktgen
    def send_packet(self, packet):
        ...
        # Print the simple_switch output
        for b_line in iter(self.proc.stdout.readline, b''):
            line = str(b_line)
            print("Line from simple_switch log: " + line.strip())
